...
def print_features(fout, x, vals):
    '''Vals is an array with more than one target.'''
    import numpy as np
    st = list_2_str(x)
    if vals is float or vals is np.float or vals is np.double:
        st += ' {0:18.12f}\n'.format(vals)
    else:
        for i in range(len(vals)):
            st += ' {0:18.12f}'.format(vals[i])
        st += '\n'
    fout.write(st)
