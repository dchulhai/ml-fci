...
#            print_features(fout, q, ehf)
            ...
            if iocc > 1:
                ...
                if val < 0.005: ldo_occ = False

            if not ldo_occ:
                print ('SKIPPED FILE: ', list_2_str(p))
                continue
            ...
            for ivir in range(1,min(nvir+1,args.vir+1)):
                ...
                for q in multiset_permutations(y):
                    ...
                    if ivir > 1:
                        ...
                        if val < 0.005: ldo = False

                    # do CASCI calculation
                    if not ldo:
                        print (list_2_str(p)+list_2_str(q))
                        continue
#                    eci = do_casci(fout, mf, mo, nao, ncore, nocc, nvir, p, q)
